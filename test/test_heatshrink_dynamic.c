...
{
    ASSERT_EQ(NULL, heatshrink_encoder_alloc(HEATSHRINK_MIN_WINDOW_BITS - 1, 8));
    ASSERT_EQ(NULL, heatshrink_encoder_alloc(HEATSHRINK_MAX_WINDOW_BITS + 1, 8));
...
TEST encoder_poll_should_reject_nulls(void)
{
    ...
    ASSERT_EQ(HSER_POLL_ERROR_NULL, heatshrink_encoder_poll(NULL, output, 256, &output_size));
    ASSERT_EQ(HSER_POLL_ERROR_NULL, heatshrink_encoder_poll(hse, NULL, 256, &output_size));
    ASSERT_EQ(HSER_POLL_ERROR_NULL, heatshrink_encoder_poll(hse, output, 256, NULL));
...
TEST encoder_sink_should_accept_input_when_it_will_fit(void)
{
    ...
    memset(input, '*', 256);
    ASSERT_EQ(HSER_SINK_OK, heatshrink_encoder_sink(hse, input, 256, &bytes_copied));
...
TEST encoder_poll_should_indicate_when_no_input_is_provided(void)
{
    ...
    HSE_poll_res res = heatshrink_encoder_poll(hse, output, 512, &output_size);
...
{
    ASSERT_EQ(NULL, heatshrink_decoder_alloc(256, HEATSHRINK_MIN_WINDOW_BITS - 1, 4));
    PASS();
}

TEST decoder_alloc_should_reject_zero_byte_input_buffer(void)
{
    ASSERT_EQ(NULL, heatshrink_decoder_alloc(0, HEATSHRINK_MIN_WINDOW_BITS, HEATSHRINK_MIN_WINDOW_BITS - 1));
    PASS();
}

TEST decoder_alloc_should_reject_lookahead_equal_to_window_size(void)
{
    ASSERT_EQ(NULL, heatshrink_decoder_alloc(0, HEATSHRINK_MIN_WINDOW_BITS, HEATSHRINK_MIN_WINDOW_BITS));
    PASS();
}
...
{
    heatshrink_decoder *hsd = heatshrink_decoder_alloc(256, HEATSHRINK_MIN_WINDOW_BITS, 4);
    size_t count = 0;
...
{
    uint8_t input[] = {0, 1, 2, 3, 4, 5};
    heatshrink_decoder *hsd = heatshrink_decoder_alloc(1, HEATSHRINK_MIN_WINDOW_BITS, HEATSHRINK_MIN_WINDOW_BITS - 1);
    size_t count = 0;
...
{
    uint8_t input[] = {0, 1, 2, 3, 4, 5};
    heatshrink_decoder *hsd = heatshrink_decoder_alloc(256, HEATSHRINK_MIN_WINDOW_BITS, HEATSHRINK_MIN_WINDOW_BITS - 1);
    size_t count = 0;
    HSD_sink_res res = heatshrink_decoder_sink(hsd, input, 6, &count);
    ASSERT_EQ(HSDR_SINK_OK, res);
...
static int compress_and_expand_and_check(uint8_t *input, uint32_t input_size, cfg_info *cfg)
{
    ...
    heatshrink_decoder *hsd =
        heatshrink_decoder_alloc(cfg->decoder_input_buffer_size, cfg->window_sz2, cfg->lookahead_sz2);
    ASSERT(hsd);
    ...
    if (cfg->log_lvl > 0) {
        printf("in: %u compressed: %zu ", input_size, polled);
    }
            ...
            if (0) {
                for (uint32_t j = 0; j <= /*i*/ input_size; j++) {
                    printf("in[%d] == 0x%02x ('%c') => out[%d] == 0x%02x ('%c')  %c\n", j, input[j],
                           isprint(input[j]) ? input[j] : '.', j, decomp[j], isprint(decomp[j]) ? decomp[j] : '.',
                           input[j] == decomp[j] ? ' ' : 'X');
                }
            }
        }
...
{
    uint8_t input[] = {'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm',
...
{
    uint8_t input[] = {'a', 'b', 'c', 'a', 'b', 'c', 'd', 'a', 'b', 'c', 'd', 'e', 'a', 'b', 'c', 'd', 'e',
                       'f', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'};
    cfg_info cfg;
    cfg.log_lvl = 0;
    cfg.window_sz2 = 8;
    cfg.lookahead_sz2 = 3;
...
TEST data_without_duplication_should_match_with_absurdly_tiny_buffers(void)
{
    ...
    if (log) {
        dump_buf("input", input, sizeof(input));
    }
    ...
    if (log) {
        dump_buf("comp", comp, packed_count);
    }
    ...
    if (log) {
        dump_buf("decomp", decomp, sizeof(input));
    }
    for (uint32_t i = 0; i < sizeof(input); i++) {
        ASSERT_EQ(input[i], decomp[i]);
    }
...
TEST small_input_buffer_should_not_impact_decoder_correctness(void)
{
    ...
    if (compress_and_expand_and_check(input, size, &cfg) != 0) {
        return -1;
    }
    PASS();
}
